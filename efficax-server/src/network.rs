...
pub async fn listen() -> (EfficaxNetwork, UnboundedReceiver<NetworkMessage>) {
    let listener = TcpListener::bind("127.0.0.1:8080").await.unwrap();

    let (tx, rx) = mpsc::unbounded_channel::<NetworkMessage>();

    let listener = tokio::spawn(async move {
        EfficaxNetwork::listen(listener, tx).await;
    });

    (EfficaxNetwork::new(listener), rx)
}
...
impl EfficaxNetwork {
    pub fn new(listener: JoinHandle<()>) -> Self {
        EfficaxNetwork { listener }
    }
    ...
    async fn listen(listener: TcpListener, tx: UnboundedSender<NetworkMessage>) {
        ...
        loop {
            ...
            tokio::spawn(async move {
                if let Err(e) = EfficaxNetwork::process(&channel, reader, addr).await {
